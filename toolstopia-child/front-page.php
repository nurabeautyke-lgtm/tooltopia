  ...
  <section class="tt-sec">
    ...
    foreach ( $tt_terms as $term ) {
        ...
        if ( '' === $img ) {
            ...
            if ( isset( $tt_cat_imgmap[ $key ] ) && file_exists( $tt_cat_dir . $tt_cat_imgmap[ $key ] ) ) {
                $img = $cat_imgs_uri . '/' . $tt_cat_imgmap[ $key ];
            } elseif ( file_exists( $tt_cat_dir . sanitize_title( $label ) . '.jpg' ) ) {
                $img = $cat_imgs_uri . '/' . sanitize_title( $label ) . '.jpg';
            } else {
                $img = $cat_imgs_uri . '/accessories.jpg';
            }
        }
        echo '<a class="tt-cat" href="' . esc_url( $link ) . '"><img class="tt-cat__img" src="' . esc_url( $img ) . '" alt="' . esc_attr( $label ) . '" width="800" height="597" loading="lazy" decoding="async"><span class="tt-cat__label">' . esc_html( $label ) . '</span></a>';
    }
    ?>
    </div>
  </section>
